...
export const data = new SlashCommandBuilder()
    .setName('light_lunch')
    .setDescription('Organize a light lunch meetup at a dining hall!')
...
export async function execute(interaction: CommandInteraction) {
    try {
        ...
        if (!mealTime) {
            await interaction.reply({
                content: 'Invalid time format. Please use formats like "2:30pm", "14:00", or "15:30".',
                ephemeral: true
            });
            return;
        }
        ...
        const eventKey = `${guildId}-${channelId}-light_lunch-${mealTime.toDateString()}`;
        if (await diningEventService.diningEventExists(eventKey)) {
            await interaction.reply({
                content: 'There\'s already an active light lunch event in this channel for that day! Wait for it to finish before starting a new one.',
                ephemeral: true
            });
            return;
        }
        ...
        const embedDescription = `**Light Lunch at ${diningHall.name} on ${dateString} at ${timeString}**\n\nJoin us for a light lunch! React to let us know if you're coming.`;
        ...
        const row = new ActionRowBuilder<ButtonBuilder>()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('light_lunch_yes')
                    .setEmoji('🥗')
                    .setLabel('I\'m In!')
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setCustomId('light_lunch_no')
                    .setEmoji('❌')
                    .setLabel('Can\'t Make It')
                    .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
                    .setCustomId('light_lunch_cancel')
        ...
        if (!eventId) {
            await interaction.editReply({
                content: 'Failed to create light lunch event. Please try again.',
                components: []
            });
            return;
        }
        ...
        collector.on('collect', async (buttonInteraction: ButtonInteraction) => {
            ...
            if (buttonInteraction.customId === 'light_lunch_yes') {
                ...
                if (userId === creator.id) {
                    ...
                    await buttonInteraction.reply({
                        content: 'You cannot cancel this light lunch event.',
                        ephemeral: true
                    });
                    return;
                }
        ...
        // Set timeout for when the light lunch time arrives
        diningEventService.setTimeout(eventKey, async () => {
            ...
            if (eventData.attendees.size === 1) {
                ...
                await channel.send(`No one else wanted to join <@${creator.id}> for light lunch at ${diningHall.name}. Event cancelled! 🥙`);
            } else {
                // Multiple people joined, send light lunch time message
                const attendeesList = Array.from(eventData.attendees.values()).map(u => `<@${u.id}>`).join(' ');
                await channel.send(`🥗 Light lunch time at ${diningHall.name}! ${attendeesList} - enjoy your meal! 🥙`);
            }
        ...
        console.error('Error executing light lunch command:', error);

        try {
            if (interaction.deferred || interaction.replied) {
                await interaction.followUp({
                    content: 'There was an error organizing the light lunch event. Please try again!',
                    ephemeral: true
                });
            } else {
                await interaction.reply({
