...
export const data = new SlashCommandBuilder()
    .setName('breakfast')
    .setDescription('Organize a breakfast meetup at a dining hall!')
    ...
    .addStringOption(option =>
        option.setName('time')
            .setDescription('What time for breakfast (e.g., "8:30am", "9:00", "08:15")')
            .setRequired(true)
    )
    .addStringOption(option =>
        option.setName('date')
            .setDescription('Date for breakfast in MM/DD/YYYY format (optional, defaults to today)')
            .setRequired(false)
    );
...
export async function execute(interaction: CommandInteraction) {
    try {
        ...
        if (!mealTime) {
            await interaction.reply({
                content: 'Invalid time format. Please use formats like "8:30am", "9:00", or "08:15".',
                ephemeral: true
            });
            return;
        }

        // Validate breakfast time range
        if (!diningEventService.isValidMealTime('breakfast', mealTime)) {
            await interaction.reply({
                content: diningEventService.getMealTimeErrorMessage('breakfast', timeInput),
                ephemeral: true
            });
            return;
        }
        ...
        // Check if there's already an active breakfast event in this channel for the same day
        const eventKey = `${guildId}-${channelId}-breakfast-${mealTime.toDateString()}`;
        if (await diningEventService.diningEventExists(eventKey)) {
            await interaction.reply({
                content: 'There\'s already an active breakfast event in this channel for that day! Wait for it to finish before starting a new one.',
                ephemeral: true
            });
            return;
        }
        ...
        const embedDescription = `**Breakfast at ${diningHall.name} on ${dateString} at ${timeString}**\n\nJoin us for breakfast! React to let us know if you're coming.`;
        ...
        const row = new ActionRowBuilder<ButtonBuilder>()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('breakfast_yes')
                    .setEmoji('🍳')
                    .setLabel('I\'m In!')
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setCustomId('breakfast_no')
                    .setEmoji('❌')
                    .setLabel('Can\'t Make It')
                    .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
                    .setCustomId('breakfast_cancel')
        ...
        if (!eventId) {
            await interaction.editReply({
                content: 'Failed to create breakfast event. Please try again.',
                components: []
            });
            return;
        }
        ...
        collector.on('collect', async (buttonInteraction: ButtonInteraction) => {
            ...
            if (buttonInteraction.customId === 'breakfast_yes') {
                ...
                if (userId === creator.id) {
                    ...
                    await buttonInteraction.reply({
                        content: 'You cannot cancel this breakfast event.',
                        ephemeral: true
                    });
                    return;
                }
        ...
        // Set timeout for when the breakfast time arrives
        diningEventService.setTimeout(eventKey, async () => {
            ...
            if (eventData.attendees.size === 1) {
                ...
                await channel.send(`No one else wanted to join <@${creator.id}> for breakfast at ${diningHall.name}. Event cancelled! 🥞`);
            } else {
                // Multiple people joined, send breakfast time message
                const attendeesList = Array.from(eventData.attendees.values()).map(u => `<@${u.id}>`).join(' ');
                await channel.send(`🍳 Breakfast time at ${diningHall.name}! ${attendeesList} - enjoy your meal! 🥞`);
            }
        ...
        console.error('Error executing breakfast command:', error);

        try {
            if (interaction.deferred || interaction.replied) {
                await interaction.followUp({
                    content: 'There was an error organizing the breakfast event. Please try again!',
                    ephemeral: true
                });
            } else {
                await interaction.reply({
