...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option =>
        option.setName('date')
            .setDescription('Date for dinner in MM/DD/YYYY format (optional, defaults to today)')
            .setRequired(false)
    );
...
export async function execute(interaction: CommandInteraction) {
    try {
        ...
        if (!diningEventService.isValidMealTime('dinner', mealTime)) {
            await interaction.reply({
                content: diningEventService.getMealTimeErrorMessage('dinner', timeInput),
                ephemeral: true
            });
            return;
        }
